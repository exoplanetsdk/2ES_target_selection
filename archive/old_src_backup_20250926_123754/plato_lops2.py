...
import numpy as np
from config import DETECTION_LIMITS, FIGURES_DIRECTORY


def plato_lops2_match(gaia_data_df):
    """
    Annotate the input DataFrame with a 'LOPS2_match' column (1 if within PLATO LOPS2 field, 0 otherwise),
    and perform the original plotting and reporting functionalities.

    Parameters:
        gaia_data_df (pd.DataFrame): Input GAIA data DataFrame.

    Returns:
        pd.DataFrame: The input DataFrame with an added 'LOPS2_match' column.
    """
    ...
    # Add the LOPS2_match column to the original input DataFrame (using index alignment)
    gaia_data_df = gaia_data_df.copy()
    # Default to 0
    gaia_data_df['LOPS2_match'] = 0
    # Only set to 1 for those with valid RA/DEC and within the field
    # Find the indices in the original DataFrame that correspond to the valid, in-field stars
    valid_indices = gaia_data.index[within_fov_mask]
    gaia_data_df.loc[valid_indices, 'LOPS2_match'] = 1

    print(f"\nTotal GAIA stars in the dataset: {len(gaia_data)}")
        ...
        if detection_limit is not None and detection_limit < 3:
            for star in filtered_stars.itertuples():
                ...
                if separation <= fov_size / 2:
                    ...
                    label = None
                    if hasattr(star, 'HD_Number') and pd.notna(star.HD_Number) and str(star.HD_Number).strip():
                        label = f"{str(star.HD_Number).strip().replace(' ', '')}"
                    elif hasattr(star, 'GJ_Number') and pd.notna(star.GJ_Number) and str(star.GJ_Number).strip():
                        label = f"{str(star.GJ_Number).strip().replace(' ', '')}"
                    elif hasattr(star, 'HIP_Number') and pd.notna(star.HIP_Number) and str(star.HIP_Number).strip():
