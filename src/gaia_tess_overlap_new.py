...
def match_gaia_tess(gaia_file, tess_file, output_file, output_merged_file, is_candidate=False, threshold_arcsec=2):
    ...
    if len(matches) > 0:
        ...
        # Get header mapping from TESS_CONFIRMED_FILE
        header_mapping = create_header_mapping_from_file(open(TESS_CONFIRMED_FILE).read())
        
        # Create merged version
        print("\nCreating merged dataset...")
        merged_df = merge_planet_data(matches_df, is_candidate)
        
        # Sort merged_df by HZ_Detection_Limit if it's confirmed planets
        if not is_candidate:
        ...
        save_and_adjust_column_widths(matches_df, output_file)
        # merged_df.to_csv(output_merged_file, index=False)
        save_and_adjust_column_widths(merged_df, output_merged_file)
