...
STELLAR_FILTERS = {
    ...
    'lum_min':      0.1,       # Minimum luminosity in solar units
    ...
    'log_rhk_max': -4.75,        # Maximum log R'HK value (for stellar activity corresponding to ~10 m/s RMS)
}
...
RESIDUAL_GRANULATION_FRACTION = 1 # granulation noise RMS residual (unlikely to be corrected due to its 49 minute decaying timescale)
RESIDUAL_SUPER_GRANULATION_FRACTION = 0 # supergranulation noise RMS residual (possible to be corrected due to its 1 day decaying timescale, depending on sampling!)

# INSTRUMENTAL_NOISE = 0.3 # m/s (instrumental noise， optimistic for HARPS-N)
# RESIDUAL_P_MODE_FRACTION = 0.1 # p-mode noise RMS residual (tested; recommended by J. Zhao)
# RESIDUAL_GRANULATION_FRACTION = 1 # granulation noise RMS residual (unlikely to be corrected due to its 49 minute decaying timescale)
# RESIDUAL_SUPER_GRANULATION_FRACTION = 1 # supergranulation noise RMS residual (possible to be corrected due to its 1 day decaying timescale, depending on sampling!)
